...
export interface LoanProduct {
  id: string;
  bankId: string; // Can also be NBFI or NGO ID
  productName: string;
...
// Additional comprehensive products for all major banks with proper structure

export const getSavingsProductsByBank = (bankId: string): SavingsProduct[] => {
  return savingsProducts.filter(product => product.bankId === bankId);
};
...
export const getLoanProductsByBank = (bankId: string): LoanProduct[] => {
  return loanProducts.filter(product => product.bankId === bankId);
};

export const getNBFIById = (nbfiId: string): NBFI | undefined => {
  return nbfis.find(nbfi => nbfi.id === nbfiId);
};

export const getNGOById = (ngoId: string): NGO | undefined => {
  return ngos.find(ngo => ngo.id === ngoId);
};

export const getAllFinancialInstitutions = () => {
  return [
    ...banks.map(bank => ({ ...bank, type: 'bank' })),
    ...nbfis.map(nbfi => ({ ...nbfi, type: 'nbfi' })),
    ...ngos.map(ngo => ({ ...ngo, type: 'ngo' }))
  ];
};